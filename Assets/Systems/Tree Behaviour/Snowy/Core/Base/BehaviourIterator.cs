...
namespace Snowy.Core.Base
{
  ...
  public class BehaviourIterator
  {
    ...
    protected readonly Utility.FixedSizeStack<int> traversal;

    // Access to the tree so we can find any node from pre-order index.
    protected readonly BehaviourTree tree;
    protected readonly Queue<int> requestedTraversals;
    ...
    protected virtual void CallOnEnterOnQueuedNodes()
    {
    ...
    protected virtual void OnChildEnter(BehaviourNode node)
    {
      if (node.Parent)
      {
    ...
    protected virtual void OnChildExit(BehaviourNode node, BehaviourNode.Status s)
    {
      if (node.Parent)
      {
    ...
    public virtual void Traverse(BehaviourNode next)
    {
    ...
    public virtual void AbortRunningChildBranch(BehaviourNode parent, int abortBranchIndex)
    {
    ...
    protected virtual void StepBackAbort()
    {
      var node = PopNode();
    ...
    internal virtual void Interrupt(BehaviourNode subtree)
    {
    ...
    protected virtual BehaviourNode PopNode()
    {
