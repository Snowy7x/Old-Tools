...
using Snowy.Core.AI.Behaviours.Utility;
using Snowy.Core.Standard;
using Snowy.Core.Utility;
using UnityEditor;
using UnityEngine;
...
namespace Snowy.Designer
{
  ...
  public class BonsaiEditor
  {
    ...
    private void DoubleClicked(object sender, BonsaiInputEvent inputEvent)
    {
      ...
      {
        // Open sub-tree
        var include = inputEvent.node.Behaviour as Include;
        if (include)
        {
          OpenIncludedSubTree(include);
        }
        
        var state = inputEvent.node.Behaviour as State;
        if (state)
        {
          OpenIncludedSubTree(state);
        }
      }
    }
    ...
    private void OpenIncludedSubTree(Include include)
    {
    ...
    private void OpenIncludedSubTree(State include)
    {
      if (include.RunningSubTree)
      {
        BonsaiWindow.OpenTree(include.RunningSubTree, Mode.View);
    ...
    public static void FetchBehaviourNodes()
    {
      ...
      {
        var nodeMeta = type.GetCustomAttribute<BehaviourNodeAttribute>(false);
