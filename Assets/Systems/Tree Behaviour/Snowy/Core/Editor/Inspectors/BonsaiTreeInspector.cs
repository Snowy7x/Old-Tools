...
namespace Snowy.Designer
{
  ...
  public class BonsaiTreeInspector : UnityEditor.Editor
  {
    ...
    public override void OnInspectorGUI()
    {
      ...
      if (target is AITree)
      {
        // Show the playerTag field.
        var playerTag = serializedObject.FindProperty("playerTag");
        EditorGUILayout.PropertyField(playerTag);
        
        // Show the nodes count.
        EditorGUILayout.LabelField("Nodes", (target as AITree).Nodes.Length.ToString());
        
        // Check if the asset has been saved yet.
        if (!(target is SubTree))
        {
          if (AssetDatabase.Contains(target))
          {
            // Show the root node.
            (target as AITree)?.TrySave();
          }
          else
          {
            EditorGUILayout.LabelField("Root", "Unset");
          }
        }
      }
